...
function chartBuild(xaxis,yaxis,ticks,type,title,orientation,color,colorscale,size,width,mode,val,wfreq)
{
  if (width == null){
    width = 500;
  }
  ...
  if (type=="gauge"){
    ...
    var trace1 = {
      ...
      type: "indicator",
      mode: mode,
      title: { text: level, font: { size: 14 },font : { color: "black" } },
      delta: { reference: maxrange/4, increasing: { color: "black" } },
      gauge: {
...
function draw(data,freq){
  ...
  var mode =null;
  var wfreq=null;
  chartBuild(xaxis,yaxis,ticks,"bar","Top 10 OTUs in Individuals","h",color,colorscale,size,null,mode,val,wfreq)
// Bubble Plot
  xaxis= data.otu_ids
  ...
  mode=null
  wfreq=null;
  chartBuild(xaxis,yaxis,ticks,"bubble","OTUs in Individuals",null,color,colorscale,size,900,mode,val,wfreq)
  ...
  val=freq;
  wfreq=freq;
  chartBuild(xaxis,yaxis,ticks,"gauge","<b>Belly Button Washing Frequency</b> <br> Scrubs per Week",null,color,colorscale,size,null,mode,val,wfreq)  
}

function guagePlot(freq){
  var level = freq*20;
  ...
  marker: {
      colors:[
          'rgba(0, 104, 14, .5)', 'rgba(10, 120, 22, .5)',
